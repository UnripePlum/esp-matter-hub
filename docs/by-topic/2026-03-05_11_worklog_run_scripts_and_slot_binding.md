...
- 운영 관점에서 `run_esp32-s3`/`open-local-web` (이 스크립트는 이후 삭제됨. `hub_api_test`로 대체) 흐름을 고정해 재현 가능한 실행 경로를 유지할 필요가 있었다.
...
- `open-local-web` 실행으로 `sta ip:` 기반 URL 오픈 확인. (이 스크립트는 이후 삭제됨. `hub_api_test`로 대체)
...
## 참고 명령
- 실행: `./run_esp32-s3`
- 웹 열기: `./open-local-web` (이 스크립트는 이후 삭제됨. `hub_api_test`로 대체)
